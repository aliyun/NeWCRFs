...
def online_eval(model, dataloader_eval, gpu, ngpus, post_process=False):
    ...
    if not args.multiprocessing_distributed or gpu == 0:
        ...
        print("{:>7}, {:>7}, {:>7}, {:>7}, {:>7}, {:>7}, {:>7}, {:>5}, {:>5}, {:>5}".format('silog', 'abs_rel', 'log10',
                                                                                            'rms',
                                                                                            'sq_rel', 'log_rms',
                                                                                            'avg_error', 'd1', 'd2',
                                                                                            'd3'))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
        writer = SummaryWriter(args.log_directory + '/' + args.model_name + '/summaries/' + args.experiment_name,
                               flush_secs=30)
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    for i in range(num_log_images):
                        measures = compute_errors(depth_gt[0][mask[0]].detach().squeeze().cpu().numpy(),
                                                  depth_est[0][mask[0]].detach().squeeze().cpu().numpy())
                        print(f'trainig - abs_rel  {measures[1]}')
                        abs_rel += measures[1]
                        # error = torch.abs(depth_gt[i, :, :, :].data - depth_est[i, :, :, :].data)
                        abs_rel_error_image = ((torch.abs(
                            depth_gt[i, :, :, :].data - depth_est[i, :, :, :].data) / depth_gt[i, :, :, :].data) * mask[
                                                   0])
                        ...
                        writer.add_image('depth_gt/image/{}'.format(i),
                                         normalize_result(1 / depth_gt_clip[i, :, :, :].data), global_step)
                    ...
                    for i in range(1):
                        ...
                        if is_best:
                            ...
                            torch.save(checkpoint,
                                       args.log_directory + '/' + args.model_name + model_save_name + '.ckpt')
                        checkpoint = {'global_step': global_step,
                                      'model': model.state_dict(),
                                      'optimizer': optimizer.state_dict(),
                                      'best_eval_measures_higher_better': best_eval_measures_higher_better,
